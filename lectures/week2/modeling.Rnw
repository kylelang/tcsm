...
%<<echo = FALSE, cache = TRUE, out.width = '\\linewidth'>>=
%p2 <- ggplot(dat1, aes(x = x, y = after_stat(density))) + theme_classic() +
%coord_cartesian(xlim = c(-4, 4))
%
%geomList <- mapply(function(x, b) geom_histogram(data = x,
%                                                 bins = b,
...
\begin{frame}{Statistical Testing}
  ...
  \begin{itemize}
    \item One way to distill this information and control for uncertainty when
      ...
      \end{itemize}
      \vc
    \item A common type of statistical test, \emph{Student's t-test}, follows this 
      pattern:
  \end{itemize}
  \begin{align*}
...
\begin{frame}{Statistical Testing}
  ...
  If we want to test the nil-null hypothesis of a zero mean difference, we get
  the following values for our example:
  \begin{align*}
...
To evaluate the extremity of our test statistics, we compare our estimated 
statistic to its null-hypothesized sampling distribution.
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item The \emph{population} is defined by an infinite sequence of repeated 
          tests.
          \begin{itemize}
            \item The sampling distribution quantifies the relative frequencies of 
              possible values the test statistic can take.
          \end{itemize}
...
@
  ...
  \end{columns}
    ...
    Nice demo: \url{http://onlinestatbook.com/stat_sim/sampling_dist/}
...
@
  ...
  \begin{frame}{Prediction \& Estimation}
    ...
    \emph{Prediction/Classification} involves building a statistical model to "guess" 
    future values of some outcome.
    \begin{itemize}
      \item Weather forcasting
    ...
    \emph{Estimation} focusses on getting the most accurate possible estimate of some 
    real-world quantity.
    \begin{itemize}
  ...
  \begin{frame}{Formal Modeling}
    ...
    \rmsc{Verbal Models}
    \begin{itemize}
      \item Vague descriptions of the theory or phenomenon
      \item The description does not describe the theory explicitly enough to
        define a single, unambiguous representation
      \item The model could describe multiple phenomena
    \end{itemize}
