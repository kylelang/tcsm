...
- By default, all latent variances and covariances are freely estimated (due to 
the default argument `auto.cov.lv.x = TRUE`)
- By default, the model is identified by fixing the first factor loading of each 
factor to 1 (due to the default argument `auto.fix.first = TRUE`)
...
Yes, a second order CFA model is a theoretically apporopriate representation of 
the *Trust* items. 
...
different aspects of an some single underlying construct. 
...
- Use the three factors defined in \@ref(l1_syntax) as the first order factors.
...
an entire additional latent variable?
...
The above represents the extent to which we will replicate the EFA/PCA components 
of Kastelä (2006) with CFA. We will now use the latent variables defined above
to revisit---for the last time---the regression analysis from Kastelä (2006).
...
the inherent constrast attirbute of the factor.
...
When we our factor only has two levels, though, the `ifelse()` function is the 
simplest way.
...
We also need to subset the data to only Finnish participants. Do so now.
...
\[
Y_{Trust_Inst} = \beta_1 X_{age} + \beta_2 X_{female} + \beta_3 X_{edu_years} +
                 \beta_4 X_{hi_pol_interest} + \beta_5 X_{lrscale} + \varepsilon_Y
\]
...
- Use the definition of the `institutions` factor from \@ref(cfa_syntax) to 
define the DV.
...
ess$trust_inst_efa <- fa(ess[7:19], 
                         nfactors = 3,          
                         rotate   = "promax",   
