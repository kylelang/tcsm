...
data comprise 143 observations of the following variables.^[These data were
simulated from the covariance matrix provided in Laible, D. J., Carlo, G., & Roesch, S. C. (2004). Pathways to self-esteem in late adolescence: The role of parent and peer attachment, empathy, and social behaviours. Journal of adolescence, 27(6), 703-716.] 
...
Suppose we are interested in the (indirect) effects of peer attachment on self-
esteem, and whether empathy has a mediating effect on this relationship.
...
###
  ...
  - Draw a path model (on paper) that can be used to test this hypothesis
  - Label the input (X), outcome (Y) and mediator/intermediary (M)
  - Label the paths a, b, and c'
  
*Hint*: Refer back to the [Mediation Basics](#mediation-basics) lecture if you need help here.
...
*Hint*: Refer back to the example in which opinions on systematic racism mediate
...
Explore the summary of the fitted model.
  ...
  - Interpret these paths in terms of regression coefficients
...
```
  ...
  The results show our regression paths for the a path (`Emp` ~ `PeerAtt`), the
  b path (`SelfEst` ~ `Emp`), and the c' path (`SelfEst` ~ `PeerAtt`).

  All three of the effects in the model, including the direct effect of
  `PeerAtt` on `SelfEst`
  ...
  $Z = `r emp_b$z %>% round(2)`$,
  $p = `r (emp_b$p / 2) %>% round(3)`$) are all positive and significant.

  We can see that the direction of the effects seems to be supportive of our
...
*Tip* You can use any labeling scheme that makes sense to you, but I recommend
  adopting some kind of systematic rule. Here, I will label the individual
  estimates in terms of the short variable names used in the path diagram above. 
...
mod <- '
## Equation for outcome:
SelfEst ~ y_m1 * Emp + PeerAtt

## Equation for mediator:
Emp ~ m1_x1 * PeerAtt

## Indirect effect:
ie_PeerAtt := m1_x1 * y_m1
'
...
pa <- tmp %>% filter(label == "ie_PeerAtt")
```

  The IE of *Peer Attachment* on *Self Esteem* through *Empathy* is significant ($\hat{\textit{IE}} = `r pa$est %>% round(3)`$, 
  $Z = `r pa$z %>% round(2)`$, $p = `r (pa$p / 2) %>% round(3)`$).
  
  (We specificed a one-sided hypothesis test!)
...
As we learned in the lecture, the default significance test for the indirect
effect here is still the same as the Sobel Z approach. To have an appropriate,
robust test of the significance of the indirect effect requires bootstrapping,
which we will do later this week in the in-class assignments. 

---

###

For now, let's add another variable to our model: *parental attachment*

Our research question now will be whether attachment, both to parents and to
peers, influences a student's self esteem. Secondarily, we would like to know
if someone's empathy influences these relationships. 

- Specify the lavaan model syntax implied by this research question
...
Now add labels to the model to allow for testing of the the indirect effect of
*PeerAtt* on *SelfEst* and *ParAtt* on *SelfEst* and estimate the model again.
...
mod3 <- '
## Equation for outcome:
SelfEst ~ y_m1 * Emp + ParAtt + PeerAtt
...
ie_ParAtt  := m1_x1 * y_m1
ie_PeerAtt := m1_x2 * y_m1
'

out <- sem(mod3, data = seData)
summary(out)
```
...
worked with for the in-class exercises last week, only this time, we will add in
a categorical variable that we will be our moderator. 

--------------------------------------------------------------------------------

### {.unnumbered}

Load the `Sesam2.sav` data.^[Data are from the very interesting study: Ball, S., & Bogatz, G. A. (1970). A Summary of the Major Findings in" The First Year of Sesame Street: An Evaluation".]
...
```{r, include = FALSE}
  ...
  srf <- round(summary(results)$fstatistic, 2)
```
...
If we compare the box plot, density, and model output below, the relationships
between the regression coefficient estimates for the viewing categories and
the group means should be evident.
...
[the previous section](#w3hhReg) (on paper).
