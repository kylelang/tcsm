...
- Provide statistical justifcation for your conclusion.
...
$\hat{Y}_{postnumb} = \beta_0 + \beta_4 X_{age}$

***VIEWCAT* 2:**

$\hat{Y}_{postnumb} = \beta_0 + \beta_1 X_{view2} + \beta_4 X_{age} + \beta_5 X_{view2} X_{age}$

***VIEWCAT* 3:**

$\hat{Y}_{postnumb} = \beta_0 + \beta_2 X_{view3} + \beta_4 X_{age} + \beta_6 X_{view3} X_{age}$

***VIEWCAT* 4:**

$\hat{Y}_{postnumb} = \beta_0 + \beta_3 X_{view4} + \beta_4 X_{age} + \beta_7 X_{view4} X_{age}$
...
get the ANCOVA-style tests that we used in \@ref(ancova_int). The marginal test
...
AN(C)OVA model are equivalent to certain model comparison tests in a regression
model. Indeed, although you cannot know this from only the examples explored
here, any significance test in an AN(C)OVA can be reformulated as a comparison
