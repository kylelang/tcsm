...
### Part 4: Linear Models and Multiple Variables
...
Sketch (on paper) a path diagram that represents the model from
[the previous section](#reg2).
...
res <- sem("POSTNUMB ~ VIEWCAT2 + VIEWCAT3 + VIEWCAT4", data = tmp)
prepare_graph(res,
...
yourself, go ahead with this next section. We will cover all the necessary
material next week in [Mediation & Moderation], but if you're interested, this
can provide some extra insight into the process.

</div>

#### Linear Models with Interactions {.unnumbered}

Based on the figure we just created, we may want to test for an interaction in
our regression model. To do so, we need to add an interaction between *AGE* and
...
To test for an interaction between *VIEWCAT* and *AGE*, we need to conduct a
hypothesis test that includes all `r nDum` interaction terms between each level
of *VIEWCAT* and *AGE*. One way to do this is to set up a hierarchical
regression analysis with three steps:

1.  Estimate the additive model wherein we regress *POSTNUMB* onto *AGE* and
    *VIEWCAT* without any interaction.
2.  Estimate the model with an interaction between *POSTNUMB* and *AGE* by
    adding it into the additive model
...
Conduct the hierarchical regression analysis described
[above](#linear-models-with-interactions])

-   Is there a significant interaction effect between *VIEWCAT* and *AGE* when
    predicting *POSTNUMB*?
