...
``` {r, eval = FALSE}
library(foreign)
ess <- read.csv("ESSround1-a.csv", to.data.frame = TRUE)
```
...
Redo the two PCAs that you conducted for the 
[At-Home Exercises](at-home-exercises-1.html) using EFA. 

- Use the `fa()` function from the `psych` package.
- Extract what you estimate to the the optimal number of factors.
- Apply an appropriate rotation.

*Hints:* 

- The eigenvalues for a given EFA solution live in the `values` slot of the 
object returned by `psych::fa()`.
- You can implement parallel analysis via `psych::fa.parallel()`.
...
## Run an EFA on the 'trust' items:
efa_trust <- fa(ess[7:19], 
                nfactors = 3, 
                rotate   = "promax",   # Oblique rotation 
                fm       = "ml",       # Estimate via maximum likelihood
                scores   = "Bartlett") # Use WLS for factor score estimation
```
