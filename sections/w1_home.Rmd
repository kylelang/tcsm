...
```{r, echo = FALSE}
library(foreign)
data <- read.spss(paste0(dataDir, "LifeSat.sav"), to.data.frame = TRUE)
```

```{r, eval = FALSE}
library(foreign)

LifeSat <- read.spss("LifeSat.sav", to.data.frame = TRUE)
```
...
- What is the ange (youngest and oldest child)? 
